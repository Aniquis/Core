...
The AniquisCoin Core daemon can be configured to act as a trusted "border
router", implementing the aniquiscoin wire protocol and relay, making
...
The ZeroMQ feature in AniquisCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building aniquiscoind:
...
For instance:
    ...
    $ aniquiscoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/aniquiscoind.tx.raw \
...
These options can also be provided in aniquiscoin.conf.
...
From the perspective of aniquiscoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into aniquiscoind directly. Furthermore, no information is
...
using. AniquisCoind appends an up-counting sequence number to each
