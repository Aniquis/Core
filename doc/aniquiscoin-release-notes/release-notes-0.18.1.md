AniquisCoin Core version 0.18.1 is now available from:

  <https://download.aniquiscoin.org/aniquiscoin-0.18.1/>
...
`/Applications/AniquisCoin-Qt` (on Mac) or `aniquiscoind`/`aniquiscoin-qt` (on
Linux).
...
AniquisCoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use AniquisCoin Core on unsupported systems.
...
10.10. Additionally, AniquisCoin Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:15929:15929` (this is an extra `:15929` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@aniquiscoin.org) the AniquisCoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 ANQS),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in ANQS per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/aniquiscoin-conf.md)
  about the `aniquiscoin.conf` file describes how to use it to configure
  AniquisCoin Core.

- A new document introduces AniquisCoin Core's BIP174 [Partially-Signed
  AniquisCoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent AniquisCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  AniquisCoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when AniquisCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of AniquisCoin Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart aniquiscoind with the `-deprecatedrpc=generate`
...
- A new `aniquiscoin-wallet` tool is now distributed alongside AniquisCoin
...
This section describes planned changes to AniquisCoin Core that may affect
other AniquisCoin software and services.

- Since version 0.16.0, AniquisCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with AniquisCoin Core 0.20 (expected about
  a year after 0.18), AniquisCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the AniquisCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in AniquisCoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any AniquisCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, AniquisCoin Core
...
- A sub-project of AniquisCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with AniquisCoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to AniquisCoin Core's own implementation, although entropy
  gathered by AniquisCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves AniquisCoin Core a
...
- On macOS, AniquisCoin Core now opts out of application CPU throttling
...
- #15929 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
