...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " aniquiscoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  aniquiscoin-tx [options] <hex-tx> [commands]  Update hex-encoded aniquiscoin transaction\n" +
            "or:     aniquiscoin-tx [options] -create [commands]   Create hex-encoded aniquiscoin transaction\n" +
            "\n";
