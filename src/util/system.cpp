...
const char * const BITCOIN_CONF_FILENAME = "aniquiscoin.conf";
...
#else
    const char* pszModule = "aniquiscoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AniquisCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/AniquisCoin";
#else
    // Unix-like
    return pathRet / ".aniquiscoin";
#endif
#endif
}
